...
namespace WoWAHDataProject.Code
{
    internal static class ConfigurationCheck
    {
        public static void InitConfigCheck()
        {
            ...
            string[] wantedKeys = { "baseDirectory", "dbDirectory", "dbFilePath", "notFirstLaunch" };
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (noDbFoundDialogResult == WinForms.DialogResult.OK)
                                {
                                    try
                                    {
                                        Database.DataBaseCreation.CreateDatabase(AppDomain.CurrentDomain.BaseDirectory + @"db\maindatabase.db", AppDomain.CurrentDomain.BaseDirectory + "db", $"Data Source={AppDomain.CurrentDomain.BaseDirectory + @"db\maindatabase.db"}");
                                    }
                                    catch (Exception ex)
                                    {
                                        Log.Error($"Error creating database: {ex.Message}");
                                        ExceptionHandling.ExceptionHandler(ex.ToString());
                                    }
                                }
                            }
                        }
        ...
        public static void CheckConfigKeys(string[] keyCollection, string[] keysToSearch, NameValueCollection appSettings, KeyValueConfigurationCollection keyValPairs, Configuration configFile, bool overwrite)
        {
            ...
            {
                if (keyCollection.Contains(key) || overwrite)
                {
                    Log.Information($"Key {key} found.");
                    Log.Information($"Value: {appSettings[key]}");
                    if (overwrite)
                    {
                        Log.Information($"Trying to overwrite value for {key}.");
                        try
                        {
                        ...
                        {
                            Log.Error($"Error overwriting value for {key}: {ex.Message}");
                        }
                    }
                }
                else
                {
                    ...
                    {
                        Log.Error($"Error adding key {key} to appSettings: {ex.Message}");
                    }
                }
